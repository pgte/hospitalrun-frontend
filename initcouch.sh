...
curl -X PUT $SECUREHOST/main/_security -d '{ "admins": { "names": [], "roles": ["admin"]}, "members": { "names": [], "roles": []}}'
...
filter="`cat server/replication_filter_function.js | tr '\\n' ' '`"
filters="{ \"filters\": { \"filter_docs_for_role\": \"${filter}\" } }"
curl -X PUT "${main}/_design/filters" -d "${filters}"
...
replicate="${SECUREHOST}/_replicator"
...
do
  ...
  curl -X PUT "${target}/_design/filters" -d "${filters}"
  curl -X POST "${replicate}" -d "{ \"_id\": \"main_to_${role}\", \"source\": \"${main}\", \"target\": \"${target}\", \"continuous\": true, \"filter\": \"filters/filter_docs_for_role\", \"query_params\": { \"role\": \"${role}\" } }" -H "content-type: application/json"
  curl -X POST "${replicate}" -d "{ \"_id\": \"${role}_to_main\", \"source\": \"${target}\", \"target\": \"${main}\", \"continuous\": true, \"filter\": \"filters/filter_docs_for_role\", \"query_params\": { \"role\": \"${role}\" } }" -H "content-type: application/json"
done
